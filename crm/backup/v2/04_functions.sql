...
-- User Management Functions
-- =====================================================================================
-- Description: Core functions for user management, onboarding, and profile handling
-- Version: 1.0
-- Dependencies: auth.users, profiles, user_preferences, user_security_settings, user_onboarding
-- =====================================================================================

-- Drop existing functions
DROP FUNCTION IF EXISTS public.handle_new_user() CASCADE;
DROP FUNCTION IF EXISTS public.handle_new_user_onboarding() CASCADE;
DROP FUNCTION IF EXISTS public.update_user_onboarding_progress() CASCADE;
DROP FUNCTION IF EXISTS public.check_onboarding_completion() CASCADE;
...
-- Notes: Creates initial profile and triggers onboarding process
...
DECLARE
    ...
    v_profile_id UUID;
BEGIN
    -- Generate initial username and handle from email
    v_username := split_part(NEW.email, '@', 1);
    v_handle := v_username;
    
    -- Create initial profile with minimal information
    ...
    INSERT INTO public.entity_emails (
        entity_id,
        entity_type,
        email,
        is_primary,
        ...
        true,
        NEW.email_confirmed,
        CASE WHEN NEW.email_confirmed THEN CURRENT_TIMESTAMP ELSE NULL END,
        'personal',
        NEW.id,
        NEW.id
    );
    ...
    PERFORM public.handle_new_user_onboarding(NEW.id, v_profile_id);
    
    RETURN NEW;
EXCEPTION
    WHEN unique_violation THEN
        RAISE NOTICE 'Unique violation occurred. Attempting to create unique handle...';
        RETURN NEW;
    WHEN OTHERS THEN
...
-- =====================================================================================
-- Description: Initializes the onboarding process for a new user
-- Called by: handle_new_user()
-- Notes: Creates necessary records in onboarding, preferences, and security tables
...
RETURNS void AS $$
BEGIN
    -- Create onboarding record with initial steps
...
-- =====================================================================================
-- Function: update_user_onboarding_progress()
-- =====================================================================================
-- Description: Updates the onboarding progress for a user
-- Parameters: 
--   p_user_id: The ID of the user
--   p_step: The current onboarding step
--   p_status: The status of the step
...
BEGIN
    ...
    -- Update step status
    v_onboarding_data := jsonb_set(
        v_onboarding_data,
        '{steps,' || p_step || ',status}',
        to_jsonb(p_status)
    );

    -- Update completed steps array if step is completed
    IF p_status = 'completed' AND NOT p_step = ANY(v_completed_steps) THEN
        v_completed_steps := array_append(v_completed_steps, p_step);
    END IF;

    -- Calculate completion percentage
    SELECT 
        (COUNT(*) FILTER (WHERE value->>'status' = 'completed') * 100 / 
         COUNT(*) FILTER (WHERE value->>'required' = 'true'))::INTEGER
    INTO v_completion_percentage
    FROM jsonb_each(v_onboarding_data->'steps');

    -- Check if all required steps are completed
    SELECT bool_and(value->>'status' = 'completed')
    INTO v_all_required_completed
    FROM jsonb_each(v_onboarding_data->'steps')
    ...
    SET 
        ...
        current_step = CASE WHEN p_status = 'completed' THEN 
            COALESCE(
                (
                    SELECT key 
                    FROM jsonb_each(v_onboarding_data->'steps') 
            ...
            ELSE current_step 
        END,
        is_completed = v_all_required_completed,
        completed_at = CASE WHEN v_all_required_completed THEN CURRENT_TIMESTAMP ELSE NULL END,
...
-- =====================================================================================
-- Function: check_onboarding_completion()
-- =====================================================================================
-- Description: Checks if a user has completed all onboarding steps
-- Parameters:
--   p_user_id: The ID of the user to check
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Error in check_onboarding_completion: %', SQLERRM;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
